...
class EcoRideMain:
  ...
  def main(self):
    ...
    This method continuously displays the menu, accepts user input, and routes the control flow to the 
    appropriate hub and vehicle management functions until the user exits the application.
    ...
    while True:
      ...
      match choice:
        ...
        case 2:
          self.input_vehicle_details()
          self.add_vehicles()
        case 3:
          for hub, vehicle_list in self.fleet_hubs.items():
        ...
        case 4:
          for vehicle in self.vehicles:
            ...
            print() 
        case 5:
          veh_battery = list(filter(lambda vehicle: vehicle.battery_percentage > 80 , self.vehicles))
          for v in veh_battery:
            print(f"{v.vehicle_id} | {v.model} | {v.battery_percentage}")
        case 6:
          self.view_vehicle_by_vehicle_type()
        case 7:
          self.vehicle_maintainance_status()
        case 8:
          hub_name = input("Enter hub name to sort vehicles: ")
          self.vehicle_sorting(hub_name)
        case 9: 
          sort_by = input("Sort by (battery/rental price):")
          self.advanced_sorting(sort_by)
        case 10: break
        case _: 
  ...
  def add_hub(self, hub_name: str):
    ...
    This method prompts user to add hub name, if the hub already exist in the fleet_hubs, a message is displayed else,
    the hub will be added to the fleet with empty list
  ...
  def add_vehicles(self):
    ...
    This method allows the user to enter multiple vehicle details (such as vehicle ID, model, and type) and assigns 
    them to an existing hub in the fleet.
    ...
    with open(filename, mode="w", newline="") as file:
      ...
      for hub, vehicle_ids in self.fleet_hubs.items():
        for vid in vehicle_ids:
          ...
          writer.writerow([hub, vehicle.vehicle_id, vehicle.model, vehicle.battery_percentage,vehicle.maintainance_status, vehicle.rental_price, 
                           vehicle_type, extra])
  ...
  def save_to_json(self, filename: str):
    ...
    print(f"Fleet data saved to JSON successfully.")
